<?php
    ...
    function LogIn($username, $password) {
        ...
        while($row = mysqli_fetch_array($result)) {
            if($username == $row['username'] & $password == $row['password']) {
                ...
                $output = '<span class="error">Log in failed</span>';
            }
        }
        return $output;
    }
    ...
    function Search($searchText, $criteria) {
        ...
        while($row = mysqli_fetch_array($result)) {
            ...
            $output .= '<li><img class="vehicleImage" src="'.$row['picture'].'" alt="Vehicle Image" height="100px"></img>'.'<p id="price">£'.$row['price'].'</p>'.'<p><b>'.$row['make'].' '.$row['model'].'</b> '.$row['year'].'</p></li>';
            $output .='</ul>';
        }
    ...
    {
        echo $value;
        list($day,$month,$year) = explode('/', $value); // handle 1992-12-12 - dash line breaks
        $month;
        $dayOptions = array(
            'options' => array(
                'min_range' => 1,
